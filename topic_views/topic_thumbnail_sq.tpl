...
<div class="thumbnail">
	{if $oTopic->getPreviewImageWebPath()}
		<a href="{$oTopic->getUrl()}"><img class="topic_preview" src="{$oTopic->getPreviewImageWebPath({cfg name='topic.thumbnail_sq.img_size'})}"></a>
	{/if}
	...
	<div class="caption">
		...
		{if {cfg name='topic.thumbnail_sm.text_length'} > 0}
			<div class="topic-content text">
				{$oTopic->getText()|strip_tags|truncate:{cfg name='topic.thumbnail_sq.text_length'}:"..."}
			</div>
		{/if}
